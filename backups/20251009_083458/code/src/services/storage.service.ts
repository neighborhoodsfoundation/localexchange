...
import { getS3Client, s3Config, storageConfig } from '../config/s3';
...
export class StorageService {
  ...
  async uploadFile(
    ...
    try {
      ...
      if (Buffer.isBuffer(data) && data.length > storageConfig.maxFileSize) {
  ...
  private isAllowedContentType(contentType: string): boolean {
    ...
    if (storageConfig.allowedTypes.length === 0) {
      return true;
    }

    return storageConfig.allowedTypes.some(allowed => {
      // Exact match
