...
Write-Host "PostgreSQL Installation Script for LocalEx" -ForegroundColor Cyan
...
Write-Host "`nInstallation Configuration:" -ForegroundColor Yellow
...
try {
    ...
    if ($pgInstalled -and !$SkipInstall) {
        Write-Host "`nPostgreSQL appears to be already installed" -ForegroundColor Yellow
    ...
    if (!$SkipInstall) {
        Write-Host "`nStep 1: Downloading PostgreSQL $($Config.Version)..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "PostgreSQL service may already be running" -ForegroundColor Yellow
    }
    ...
    Write-Host "`nStep 4: Creating database and user..." -ForegroundColor Yellow
    ...
    Write-Host "`nStep 6: Creating environment configuration..." -ForegroundColor Yellow
...
"@
    ...
    Write-Host "`nStep 7: Installing npm dependencies..." -ForegroundColor Yellow
    ...
    if (Test-Path "package.json") {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "npm install failed - you may need to run 'npm install' manually" -ForegroundColor Yellow
        }
    } else {
    ...
    # Success message
    Write-Host "`nPostgreSQL Installation Complete!" -ForegroundColor Green
