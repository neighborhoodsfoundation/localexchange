...
export const TradingProvider: React.FC<TradingProviderProps> = ({ children }) => {
  ...
  const makeOffer = async (_request: MakeOfferRequest): Promise<MakeOfferResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const respondToOffer = async (_request: RespondToOfferRequest): Promise<RespondToOfferResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      ...
      const mockTrade: Trade = {
        ...
        sellerId: 'seller-123',
        offeredPrice: 100,
        status: _request.response === 'ACCEPT' ? 'OFFER_ACCEPTED' as any : 'OFFER_REJECTED' as any,
        createdAt: new Date(),
        updatedAt: new Date()
      };
  ...
  const getOffers = async (_tradeId: string): Promise<TradeOffer[]> => {
    try {
  ...
  const searchSafeZones = async (_request: SafeZoneSearchRequest): Promise<SafeZoneSearchResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const selectLocation = async (_request: SelectLocationRequest): Promise<SelectLocationResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const getLocationRecommendations = async (_tradeId: string): Promise<any> => {
    try {
  ...
  const selectTime = async (_request: SelectTimeRequest): Promise<SelectTimeResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const getTimeSuggestions = async (_tradeId: string): Promise<Date[]> => {
    try {
  ...
  const createEscrow = async (_request: CreateEscrowRequest): Promise<CreateEscrowResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const releaseEscrow = async (_tradeId: string): Promise<boolean> => {
    try {
  ...
  const refundEscrow = async (_tradeId: string): Promise<boolean> => {
    try {
  ...
  const confirmArrival = async (_request: ConfirmArrivalRequest): Promise<ConfirmArrivalResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const confirmHandoff = async (_request: ConfirmHandoffRequest): Promise<ConfirmHandoffResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const getArrivalStatus = async (_tradeId: string): Promise<any> => {
    try {
  ...
  const leaveFeedback = async (_request: LeaveFeedbackRequest): Promise<LeaveFeedbackResponse> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const getFeedback = async (_userId: string): Promise<any[]> => {
    try {
  ...
  const disputeTrade = async (tradeId: string, _issueType: string, _description: string): Promise<boolean> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const canPerformAction = async (_tradeId: string, _action: string): Promise<boolean> => {
    try {
