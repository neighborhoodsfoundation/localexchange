...
export const useTrade = (tradeId: string) => {
  const { getTrade, isLoading, error } = useTrading();
...
export const useUserTrades = (userId: string, status?: TradeStatus[]) => {
  ...
  const loadTrades = useCallback(async () => {
    ...
    try {
      const response = await getTrades({ userId, status: status || [] });
 ...
 */
export const useLocationSelection = (_tradeId: string) => {
...
export const useArrivalTracking = (tradeId: string) => {
  ...
  const confirmUserArrival = useCallback(async (_userId: string, locationLat?: number, locationLng?: number) => {
...
export const useHandoffConfirmation = (tradeId: string) => {
  ...
  const confirmHandoffForTrade = useCallback(async (_userId: string, itemAsDescribed: boolean, issues?: string[]) => {
    setIsConfirming(true);
    try {
