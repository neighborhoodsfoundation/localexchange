...
export class CDNService {
  ...
  private getCDNConfig(): CDNConfig {
    ...
    return {
      ...
      domain: (s3Config as any).cdnUrl || '',
      enabled: !!(s3Config as any).cdnUrl,
    };
  }
   ...
   */
  getCacheHeaders(_key: string, policy?: CachePolicy): Record<string, string> {
   ...
   */
  async getCacheHitRatio(_timeRange: number = 3600000): Promise<number> {
    try {
  ...
  async getCDNStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        ...(this.config.distributionId && { distributionId: this.config.distributionId }),
        cacheHitRatio,
