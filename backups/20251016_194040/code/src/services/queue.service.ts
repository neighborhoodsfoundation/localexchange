...
import { queueRedis, QUEUE_CONFIG, redisUtils } from '../config/redis';
...
class QueueService extends EventEmitter {
  ...
  async addJob(
    ...
    try {
      ...
      // Add optional delay if provided
  ...
  private async processJob(queueName: string, jobId: string): Promise<void> {
    try {
      ...
      const job = redisUtils.deserialize(jobData) as JobData | null;
      if (!job) {
        console.error('Failed to deserialize job data');
        return;
      }
