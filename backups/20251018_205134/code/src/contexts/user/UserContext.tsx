...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
  ...
  const updatePrivacy = async (privacy: Partial<PrivacySettings>): Promise<boolean> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const submitVerification = async (documents: any): Promise<boolean> => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
  ...
  const getIdentityForTrade = async (tradeId: string): Promise<any> => {
    try {
