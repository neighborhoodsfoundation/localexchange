...
import { imageConfig, storageConfig } from '../config/s3';
...
export class ImageProcessingService {
  ...
  async validateImage(buffer: Buffer): Promise<{ valid: boolean; error?: string; metadata?: ImageMetadata }> {
    try {
      ...
      if (buffer.length > storageConfig.maxFileSize) {
        return {
          valid: false,
          error: `Image size ${buffer.length} bytes exceeds maximum ${storageConfig.maxFileSize} bytes`,
        };
      }
  ...
  async processImage(buffer: Buffer, options: ProcessingOptions = {}): Promise<Buffer> {
    try {
      ...
      if (options.stripExif !== false && imageConfig.stripExif) {
  ...
  async generateThumbnails(buffer: Buffer): Promise<{ [key: string]: Buffer }> {
    const sizes: ImageSize[] = [
      { name: 'thumbnail', width: imageConfig.thumbnailSize, fit: 'cover' },
      { name: 'medium', width: imageConfig.mediumSize, fit: 'inside' },
      { name: 'large', width: imageConfig.largeSize, fit: 'inside' },
    ];
